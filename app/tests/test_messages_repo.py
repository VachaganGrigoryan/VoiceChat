...
async def test_create_and_list_history():
    ...
    try:
        ...
        items, next_cursor = await repo.list_history(user_id=u1, peer_user_id=u2, limit=10)
        assert len(items) == 1
        assert next_cursor is not None

        items2, next2 = await repo.list_history(user_id=u1, peer_user_id=u2, limit=10, cursor=next_cursor)
        assert items2 == []
