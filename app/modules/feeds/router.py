...
@router.get(
    "/users/{user_id}",
    response_model=PaginatedResponse[list[FeedPostView]],
    dependencies=[Depends(rate_limit("30/minute", scope="feed_user_aggregate"))],
)
async def list_user_feed(
    ...
    items, next_cursor = await service.list_user_feed(
        viewer_id=user.str_id, owner_id=user_id, limit=limit, cursor=cursor
    )
    return ok_paginated(
        request,
        data=items,
