...
async def get_active_call(
    ...
    payload = await service.get_active_call_session(user_id=user.str_id)
...
async def reject_call(
    ...
    result = await service.reject_call(user_id=user.str_id, call_id=call_id)
    model = result.call
    cancel_call_expiration(call_id)
    cancel_call_reconnect_timeout(call_id)
...
async def end_call(
    ...
    result = await service.end_call(user_id=user.str_id, call_id=call_id)
    model = result.call
    cancel_call_expiration(call_id)
    cancel_call_reconnect_timeout(call_id)
    await clear_call_bindings(
